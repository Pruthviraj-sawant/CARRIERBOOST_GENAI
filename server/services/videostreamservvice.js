...
const generateJavaQuestion = async (difficulty = 'Easy') => {
  const API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GEMINI_API_KEY}`;
    ...
    contents: [
      {
        parts: [
          {
            text: `Generate ONLY one Java programming interview question based on the given difficulty. 
The question must be strictly related to Java programming concepts or data structures in Java.

Instructions:
- No explanation or answer
...
- Topic examples include: 
  ...
  - Multithreading & Concurrency (Thread class, Runnable, ExecutorService)
  - Java Streams and Lambdas
  - Java Memory Management & Garbage Collection
  - JVM Internals
  - Exception Handling
  - Java-based Data Structures (LinkedList, Stack, Queue, Tree, Graph, Trie, Heap, etc.)
  -TREE,GRAPH,LINKED LIST,STACK,QUEUE,HEAP,TRIE
- Difficulty Levels: Easy, Intermediate, Advanced, Legendary
...
Difficulty Level: ${difficulty}`
  ...
  try {
    ...
    console.error('Error while calling Gemini API:', error.response?.data || error.message);
    throw new Error('Failed to generate Java interview question.');
  }
};

module.exports = { generateJavaQuestion };
